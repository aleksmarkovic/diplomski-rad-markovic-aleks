...
\qquad Izmeðu navedena dva koraka, nalazi se ROS\#, most koji spaja ROS i Unity. ROS\# pomoæu RosBridgeClient-a (pojasnit ROSBRIDGECLIENT) otvara vezu izmeðu njih. Isti radi na temelju pretplate i izdavanja poruka na ROS teme (topic). Jednom kad se veza otvorila, Unity se može pretplaæivati na ROS teme i izdavati nove poruke na ROS teme.
...
\begin{itemize}
	...
	\item Prefix robota - opcionalno polje u sluèaju da ima više robota u simulaciji. Pod prefix se podrazumijeva prefix ROS tema ispred teme robota. Npr. postoji simulacija s više Turtlebot-ova gdje svaki Turtlebot ima svoje ROS teme kojeg su inaèe istog naziva, njima je potrebno dodati prefix ispred naziva ROS teme da bi se znalo na kojeg Turtlebot-a se ta ROS tema odnosi (slika \ref{fig:multirobotexample}). Prefix je potrebno definirati u glavnoj \emph{launch} datoteci simulacije. (PRILOG ??)
\end{itemize}
...
\begin{itemize}
	...
	\item \emph{Odometry Subscriber} koji se pretplaæuje na temu \emph{/odom} i dohvaæa odometrijske podatke robota. Dobivene podatke skripta koristi za ažuriranje odometrijskih podataka robotskog modela u Unity prostoru.
...
\qquad Navedeni objekt služi samo za pokretanje prethodno navedene skripte koja omoguæuje prikaz više robota odjednom.
\clearpage

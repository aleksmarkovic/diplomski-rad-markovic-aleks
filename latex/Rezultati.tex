...
\qquad Implementacija se može podijeliti u 2 dijela, faza 1 i faza 2. Faza 1 podrazumijeva upravljanje Turtlebot Waffle Pi robotom, prikaz s kamera, prikaz podataka laserskog skena i 2D mapiranje. Faza 2 je iskljuèivo 3D mapiranje, uz upravljanje Turtlebot Waffle robota. Ona je zasebno implementirana u drugoj sceni iz razloga što je stvaranje i prikaz 3D mape resursno zahtjevnije, ali ovom podijelom je i sam prikaz uredniji.
...
\begin{figure}[!htbp]
	\begin{center}
		...
		\caption{Windows aplikacija - 2D mapa, robot i laserski sken}
		\label{fig:laserscan}
	\end{center}
\end{figure}
...
\begin{figure}[!htbp]
	\begin{center}
		...
		\caption{Mobilna aplikacija - 2D mapa, robot i laserski sken}
		\label{fig:a3}
	\end{center}
\end{figure}
\newpage
...
\begin{enumerate}
	...
	\item \textbf{Pozicioniranje mape u Unityju}
	
	Pozicioniranje mape u Unityju podrazumijeva na koji se naèin slika mape nacrta na \emph{Plane} objekt, gdje se je relativno ogranièeno zbog naèina èitanja \emph{nav\_msgs/OccupancyGrid} podataka te slijedno tome crtanju teksture gdje je vrlo teško napraviti precizan pomak.
	
	Mapa se generira sukladno SLAM gmappingu pa je ona veæinom toèna. No kad se pozicija mape poremeti, daljnjim skeniranjem (šetanjem) prostora ona se u veæini sluèajeva vrati u normalnijim okvirima (ilustracija \ref{fig:mappingsteps}). Takoðer, testiranjem se utvrdilo da je pozicija robota toèna u Unity prostoru.	
...
Cilj ove faze bila je izrada 3D mape. Na slikama \ref{fig:3d1} i \ref{fig:3d2} prikazana je 3D mapa prostora u Unity Editoru. Slobodno šetanje po mapi moguæe je samo u Editoru, dok u samostalnim aplikacijama kamera slijedi robota.
...
\begin{enumerate}
	...
	\item Funkcija za generiranje novih objekata u prostoru \emph{Create3DMap()} odnosno ažuriranje mape - generira nove objekte u prostoru po koordinatama u novoj listi koja sadrži samo nove toèke. Takoðer dodaje imena novih objekata u prikladnu listu.
\end{enumerate}

...
\qquad Prije samog rješavanja problematike kako napraviti navedenu aplikaciju, potrebno je objasniti što su i kako funkcioniraju korišteni softverski alati. Definirat æe se i koji su preduvjeti, tj. knjižnice ili alati koji zahtijevaju izvršavanje zadane funkcije.
...
Glavni naèin komunikacije kroz ROS vrši se putem ROS tema (eng. topic) \cite{rostopic}. ROS teme su \emph{"kanali"} putem kojih ROS èvorovi (eng. nodes) izmijenjuju podatke. ROS èvor je proces koji odraðuje odreðeni zadatak \cite{rosnode}. Èvor objavljuje podatke na odreðenu temu, a drugi se èvor pretplaæuje na istu temu da bi dobivao objavljene podatke te jedna tema može imati više èvorova koji su pretplaæeni ili objavljuju na nju. Svaka ROS tema ima odreðeni tip podataka i može primati i slati podatke samo tog formata - ROS poruka. ROS poruka je jednostavni oblik strukture podataka s odreðenim tipovima varijabla (primitivni tipovi, polja primitivnih tipova i ugniježðeni tipovi podataka) \cite{rosmessage}.

\emph{roslaunch} je ROS alat koji omoguæuje pokretanje više ROS èvorova odjednom iz \emph{.launch} datoteka XML formata \cite{roslaunch}. U njemu se mogu definirate sve potrebne stvari potrebne za pokretanje simulacije, npr. pokretanje i uèitavanje simuliranog okruženja, pokretanje potrebnih èvorova te definiranje potrebnih argumenata. \emph{roslaunch} automatski pokreæe i \emph{roscore} - ROS znaèajka koja služi kao jezgra svih ROS èvorova koja mora postojati da bi ROS èvorovi mogli komunicirati. \cite{roscore}

Za programiranje novih ROS èvorova podržani su Python i C++ programski jezici.
...
Cilj \textit{Mono} razvojnog okruženja je da omoguæi korištenje .NET Frameworka na razne operacijske sustave, jer je inaèe .NET framework razvijen od strane Microsofta samo za Windows OS. Komponente \textit{Mono} frameworka ukljuèuju:
\begin{enumerate}
...
Glavne znaèajke \textit{Mono} radnog okruženja:
\begin{itemize}
...
Programiranje prilagoðenih skripti omoguæava kompleksna ponašanja i radnje u igri/softveru kojega se razvija. Gotovo sve akcije koje se mogu napraviti u Editoru, mogu se i u skripti, te se time može drastièno poveæati opseg moguænosti.
\clearpage

\section{ROS\#}
\qquad ROS\# je set programskih knjižnica i alata otvorenog koda u C\#u koja omoguæuje komunikaciju izmeðu ROSa i .NET aplikacija - Unityja, razvijen od strane Siemens kompanije. Siemens je kompanija koja se bavi razvijanjem tehnologije za bitne ekonomske industrije (transport, tvornice i sl.). \cite{siemens}
...
Glavni sadržaj ovog paketa je sljedeæi:
\begin{enumerate}
...
\qquad Gazebo je alat za 3D simulaciju robota i njegovog okruženja kojega æe se u radu koristiti za pokretanje Turtlebot simulacije (slika \ref{fig:gazebo}). Neke od bitnijih znaèajki ovog alata su: realistièna fizika u simulaciji, napredna 3D grafika i efekti, stvaranje senzornih podataka, prikaz robotskih modela i slièno.  \cite{gazebosim}

Gazebo simulaciju æe se pokretati \emph{roslaunch} naredbom nad datotekom ekstenzije \emph{.launch} koja sadrži potrebne parametre simulacije, od kojih je za Gazebo bitna lokacija \emph{.world} datoteke. \emph{World} datoteka opisuje model svijeta tj. okoline koja æe se simulirati. \cite{gazeboroslaunch}

\begin{figure}[!htbp]
	\begin{center}
		\includegraphics[height=12cm,width=14cm,keepaspectratio=true]{gazebo}
		\caption{Gazebo}
		\label{fig:gazebo}
	\end{center}
\end{figure}
